...
TEST(FactoryIOTest, ParseLinkOneReceiverDwa) {
...
TEST(FactoryIOTest, ParseLinkOneReceiver) {
    ...
    auto prefs = r.receiver_preferences_.get_preferences();
    ASSERT_EQ(1U, prefs.size());
    auto key = dynamic_cast<IPackageReceiver*>(const_cast<Worker*>(&w));
    ASSERT_NE(prefs.find(key), prefs.end());
    EXPECT_DOUBLE_EQ(prefs[key], 1.0);
}

TEST(FactoryIOTest, ParseLinkOneReceiverWorker) {
    std::ostringstream oss;
    ...
    ASSERT_EQ(std::next(factory.worker_cbegin(), 2), factory.worker_cend());
    const auto& w1 = *(factory.worker_cbegin());
    const auto& w2 = *(std::next(factory.worker_cbegin(), 1));

    auto prefs = w2.receiver_preferences_.get_preferences();
    ASSERT_EQ(2U, prefs.size());
    auto key1 = dynamic_cast<IPackageReceiver*>(const_cast<Worker*>(&w1));
    auto key2 = dynamic_cast<IPackageReceiver*>(const_cast<Worker*>(&w2));
...
TEST(FactoryIOTest, ParseLinkMultipleReceivers) {
    std::ostringstream oss;
    oss << "WORKER id=1 processing-time=1 queue-type=FIFO" << "\n"
        << "STOREHOUSE id=1" << "\n"
        << "STOREHOUSE id=2" << "\n"
    ...
    auto prefs = w.receiver_preferences_.get_preferences();
    ASSERT_EQ(2U, prefs.size());
...
TEST(FactoryIOTest, ParseLinkMultipleReceiversDwa) {
    std::ostringstream oss;
    oss << "LOADING_RAMP id=1 delivery-interval=3" << "\n"
        << "STOREHOUSE id=1" << "\n"
        << "STOREHOUSE id=2" << "\n"
